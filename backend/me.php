...
$db = get_db();
$stmt = $db->prepare('SELECT id, full_name, personal_email, department, role, avatar_url, is_field_employee, date_of_birth, vari_points FROM employees WHERE id = ? LIMIT 1');
$stmt->execute([$myId]);
$emp = $stmt->fetch();
