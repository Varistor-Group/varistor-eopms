...
$sql = [string](Get-Content -Path "supabase\rpc_delete_employee.sql" -Raw)
$body = @{ query = $sql } | ConvertTo-Json -Depth 5 -Compress

Write-Host "Deploying delete RPC function..."
try {
    Invoke-RestMethod -Uri $dbUrl -Method POST -Headers $headers -Body $body -ContentType "application/json" | Out-Null
    Write-Host "Delete RPC function deployed successfully!"
} catch {
