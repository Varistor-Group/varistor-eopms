...
VALUES ('VAR-001',12,0,10,0,15,0,0),('VAR-002',12,0,10,0,15,0,0),('VAR-003',12,0,10,0,15,0,0)
ON CONFLICT (employee_id) DO NOTHING;
"@

# 3. Holidays
Run-SQL "Holidays" @"
INSERT INTO public.holidays (date, occasion, type, apply_to_all, created_by) VALUES
...
# 4. Announcements
Run-SQL "Announcements" @"
INSERT INTO public.announcements (id, title, content, author_role, type, created_at) VALUES
('ann-1','New POSH policy v2.1 published','We have updated our Prevention of Sexual Harassment (POSH) guidelines. All employees are required to review the document and complete the mandatory training module.','HR','Policy',NOW() - INTERVAL '2 hours'),
('ann-2','Quarterly town-hall this Friday','Join us for our Q2 performance review town-hall this Friday at 3:00 PM in the main conference room.','HR','Standard',NOW() - INTERVAL '5 hours'),
('ann-3','Office closed on 2nd Oct','In observance of Gandhi Jayanti, the office will remain closed on October 2nd.','HR','Standard',NOW() - INTERVAL '3 days')
ON CONFLICT (id) DO NOTHING;
"@

# 5. Policies
Run-SQL "Policies" @"
INSERT INTO public.policies (id, title, target, content, effective_date) VALUES
...
('mod-gen-1','Welcome & Company Values','An introduction to Varistor Technologies — our mission, culture, and what we stand for as a team.','General',NULL,480,'https://images.unsplash.com/photo-1552664730-d307ca884978?w=640&fit=crop&q=60','https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4',1,NULL,'{}',true),
('mod-gen-2','General Behaviour & POSH','Workplace conduct standards and the Prevention of Sexual Harassment (POSH) policy — mandatory for all employees.','General',NULL,750,'https://images.unsplash.com/photo-1521737604893-d14cc237f11d?w=640&fit=crop&q=60','https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ElephantsDream.mp4',2,'mod-gen-1','{}',true),
('mod-gen-3','Data Security & IT Policy','How to handle company data responsibly — acceptable use of devices, password hygiene, and incident reporting.','General',NULL,600,'https://images.unsplash.com/photo-1563986768609-322da13575f3?w=640&fit=crop&q=60','https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerBlazes.mp4',3,'mod-gen-2','{}',true),
('mod-dept-1','Department Orientation — Operations SOPs','Standard Operating Procedures for the Operations department.','Department','Operations',900,'https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=640&fit=crop&q=60','https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerEscapes.mp4',1,NULL,'{}',true),
('mod-dept-2','Tool Onboarding — EOPMS Walkthrough','A hands-on walkthrough of the Varistor EOPMS platform.','Department','Operations',660,'https://images.unsplash.com/photo-1497366216548-37526070297c?w=640&fit=crop&q=60','https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerFun.mp4',2,'mod-dept-1','{}',true),
('mod-tech-1','Technical Training — Role Specific','Role-specific technical onboarding covering the tools and internal systems.','Tech',NULL,1200,'https://images.unsplash.com/photo-1517694712202-14dd9538aa97?w=640&fit=crop&q=60','https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerJoyrides.mp4',1,NULL,'{}',true)
ON CONFLICT (id) DO NOTHING;
"@

# 7. Quiz questions
Run-SQL "Quiz Questions" @"
...
('q1-2','mod-gen-1','Which value is at the core of Varistor''s culture?',ARRAY['Speed above all else','Collaboration and transparency','Individual recognition only','Cost minimization'],1),
...
('q2-3','mod-gen-2','The POSH Act applies to:',ARRAY['Only female employees','Only permanent employees','All employees regardless of gender or contract type','Only senior management'],2),
...
('q3-1','mod-gen-3','You receive a suspicious email asking for your EOPMS password. What do you do?',ARRAY['Reply with your password','Forward it to HR/IT immediately','Ignore it and continue working','Share it with a colleague to check'],1),
('q3-2','mod-gen-3','Which of the following is an acceptable password practice?',ARRAY['Using your name and birthdate','Sharing passwords with your manager','Using a unique password with >12 characters','Reusing old passwords'],2),
('q3-3','mod-gen-3','Company data must be stored:',ARRAY['On personal Google Drive','On company-approved systems only','On USB drives for easy sharing','Locally only, never in cloud'],1),
('q4-1','mod-dept-1','Daily task updates must be submitted by:',ARRAY['9 AM','12 PM (noon)','End of day','Weekly on Friday'],2),
...
Write-Host "`nAll seed operations complete."
