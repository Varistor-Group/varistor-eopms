...
import { Bell, Megaphone, Cake, MessageSquare, CheckCheck } from 'lucide-react';
...
export const NotificationBell: React.FC<NotificationBellProps> = ({ onNavigateToChat }) => {
  const { announcements, readAnnouncement, reactToAnnouncement } = useVariPoints();
  ...
  const totalUnread = unreadAnnouncements.length + chatUnread.total;
