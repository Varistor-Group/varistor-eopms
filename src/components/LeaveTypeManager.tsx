  ...
  return (
    <div className="bg-white rounded-varistor border border-varistor-border p-5 shadow-varistor">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          <h4 className="text-sm font-bold text-varistor-muted mb-2.5 uppercase">Create New Type</h4>
        ...
        <div className="lg:border-l lg:border-varistor-border lg:pl-6">
          <h4 className="text-sm font-bold text-varistor-muted mb-2.5 uppercase">Existing Leave Types</h4>
          {loading ? (
            <div className="text-sm text-varistor-muted">Loading...</div>
          ) : (
            <div className="overflow-x-auto border border-varistor-border rounded-lg">
              <table className="w-full text-sm text-left">
                <thead className="bg-varistor-pageBg border-b border-varistor-border text-xs text-varistor-muted uppercase">
                  <tr>
                    <th className="px-3 py-2 font-bold">Name</th>
                    <th className="px-3 py-2 font-bold">Allocation</th>
                ...
                <tbody>
                  {leaveTypes.map(lt => (
                    <tr key={lt.id} className="border-b border-varistor-border last:border-0 hover:bg-varistor-pageBg">
                      <td className="px-3 py-2.5">
                        ...
                        {lt.description && <div className="text-xs text-varistor-muted">{lt.description}</div>}
                      </td>
                      <td className="px-3 py-2.5 whitespace-nowrap">{lt.default_allocation} days</td>
                  ...
                  {leaveTypes.length === 0 && (
                    <tr>
                      <td colSpan={3} className="px-3 py-8 text-center text-varistor-muted">No custom leave types found.</td>
                    </tr>
                  )}
                </tbody>
              </table>
